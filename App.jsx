...
function Logo({ size = 36 }) {
  ...
  if (err) return (
    <div style={{ width: size, height: size, borderRadius: "50%", background: O, display: "flex", alignItems: "center", justifyContent: "center", color: "#fff", fontSize: size * 0.28, fontWeight: 700, flexShrink: 0 }}>SRK</div>
  );
    ...
    <div style={{ background: N, padding: "36px 14px 0" }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 8 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          <Logo size={34} />
          <div>
            <div style={{ color: "#fff", fontSize: 13, fontWeight: 700 }}>SRK TAX AUDIT</div>
            <div style={{ color: "rgba(255,255,255,0.45)", fontSize: 9 }}>Pillars of Your Business</div>
          </div>
        </div>
      ...
      {showClient && client && (
        <div style={{ background: "rgba(255,255,255,0.1)", borderRadius: 10, padding: "8px 11px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <div>
            <div style={{ color: "#fff", fontSize: 11, fontWeight: 600 }}>{client.name}</div>
            <div style={{ color: "rgba(255,255,255,0.4)", fontSize: 9, fontFamily: "monospace" }}>{client.gstin || client.phone}</div>
          </div>
...
function BottomNav({ tab, setTab }) {
  const items = [
    ...
    { id: "upload", ico: "📤", lbl: "Upload" },
    { id: "returns", ico: "📋", lbl: "Returns" },
    { id: "payment", ico: "💳", lbl: "Payment" },
    { id: "dues", ico: "📅", lbl: "Dues" },
...
function Card({ children, style = {} }) {
  return <div style={{ background: "#fff", borderRadius: 11, padding: "9px 10px", marginBottom: 8, border: "0.5px solid #e6ecf5", ...style }}>{children}</div>;
}
...
function SL({ children, color = N }) {
  return <div style={{ fontSize: 9, fontWeight: 700, color, textTransform: "uppercase", letterSpacing: "0.3px", marginBottom: 5 }}>{children}</div>;
}
  ...
  return (
    <div style={{ flex: 1, padding: 10, background: "#f2f5fa", overflowY: "auto" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 6, marginBottom: 6 }}>
        ...
        <SC icon="📤" label="Uploaded" value={pu || "—"} sub={pu ? "New files" : "No new uploads"} accent="#7c3aed" vc="#7c3aed" />
      </div>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 6, marginBottom: 6 }}>
        <SC icon="📋" label="Returns" value={pr || "—"} sub={pr ? "Pending" : "All clear"} accent="#0c447c" vc="#0c447c" />
      ...
      <div onClick={() => setTab("dues")} style={{ background: "#fff", borderRadius: 10, padding: "8px 10px", border: "0.5px solid #e6ecf5", borderLeft: "3px solid #b91c1c", marginBottom: 8, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
          ...
          <div>
            <div style={{ fontSize: 8, color: "#8896b0" }}>Due Dates</div>
            <div style={{ fontSize: 14, fontWeight: 700, color: "#b91c1c" }}>{ud} urgent</div>
          </div>
        </div>
      ...
      <Card>
        ...
        {NOTICES_DATA.filter(n => n.status !== "Closed").map((n, i) => (
          <div key={i} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "5px 0", borderBottom: i < 1 ? "0.5px solid #f0f4fa" : "none" }}>
            <div>
              <div style={{ fontSize: 11, fontWeight: 600, color: "#1a2440" }}>{n.title}</div>
              <div style={{ fontSize: 9, color: "#8896b0" }}>{n.sub}</div>
            </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr", gap: 5 }}>
        {[["📬", "Inbox", "inbox"], ["🤖", "Ask SRK", "ask"], ["📷", "Vision", "vision"], ["⚠️", "Notices", "notices"]].map(([ico, lbl, id]) => (
          <button key={id} onClick={() => setTab(id)} style={{ background: "#fff", borderRadius: 9, border: "0.5px solid #e6ecf5", padding: "8px 4px", textAlign: "center", cursor: "pointer" }}>
            ...
            <div style={{ fontSize: 8, fontWeight: 600, color: "#1a2440" }}>{lbl}</div>
          </button>
        ))}
      </div>
    </div>
...
// ── Upload screen (with real file storage) ────────────────────────────────────
    ...
    <div style={{ flex: 1, padding: 10, background: "#f2f5fa", overflowY: "auto" }}>
      <Card>
        ...
        <div style={{ display: "flex", flexWrap: "wrap", gap: 4 }}>
          {CATS.map(c => (
            <button key={c} onClick={() => setCat(c)} style={{ padding: "4px 9px", borderRadius: 14, border: "0.5px solid", borderColor: cat === c ? N : "#e6ecf5", background: cat === c ? N : "#f4f7fb", color: cat === c ? "#fff" : "#8896b0", fontSize: 9, fontWeight: 600, cursor: "pointer" }}>{c}</button>
          ))}
        </div>
      </Card>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 6, marginBottom: 8 }}>
        <div>
          <SL>Financial Year</SL>
          <select value={fy} onChange={e => setFy(e.target.value)} style={{ width: "100%", border: "0.5px solid #e6ecf5", borderRadius: 8, padding: "7px 9px", fontSize: 10, background: "#fff", outline: "none", color: "#1a2440" }}>
            <option>2025-26</option><option>2024-25</option><option>2023-24</option>
          </select>
        </div>
        <div>
          <SL>Remarks</SL>
          <input value={rmk} onChange={e => setRmk(e.target.value)} placeholder="Optional…" style={{ width: "100%", border: "0.5px solid #e6ecf5", borderRadius: 8, padding: "7px 9px", fontSize: 10, background: "#fff", outline: "none", color: "#1a2440" }} />
        </div>
      </div>
      ...
      {files.length > 0 && (
        <Card style={{ marginBottom: 8 }}>
          {files.map((f, i) => (
            <div key={i} style={{ display: "flex", alignItems: "center", gap: 6, padding: "5px 0", borderBottom: i < files.length - 1 ? "0.5px solid #f0f4fa" : "none" }}>
              <span style={{ fontSize: 8, fontWeight: 700, padding: "2px 5px", borderRadius: 4, background: "#fde8e6", color: "#b91c1c" }}>{ext(f.name).toUpperCase()}</span>
              <span style={{ flex: 1, fontSize: 10, fontWeight: 600, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{f.name}</span>
              <span style={{ fontSize: 9, color: "#8896b0" }}>{fs(f.size)}</span>
              <button onClick={() => setFiles(p => p.filter((_, j) => j !== i))} style={{ color: "#ccc", background: "none", border: "none", cursor: "pointer", fontSize: 12, fontWeight: 700 }}>×</button>
            </div>
          ))}
        </Card>
      )}
      {busy && (
        <div style={{ marginBottom: 8 }}>
          <div style={{ fontSize: 9, color: "#8896b0", marginBottom: 4 }}>Uploading {pct}%…</div>
          <div style={{ background: "#f0f3fa", borderRadius: 6, height: 5, overflow: "hidden" }}>
            <div style={{ width: pct + "%", height: "100%", background: O, borderRadius: 6, transition: "width 0.3s" }} />
          </div>
        </div>
      )}
...
// ── Returns screen (LIVE from Supabase filings table) ─────────────────────────
function ReturnsScreen({ client }) {
  ...
  useEffect(() => {
    ...
    db(`filings?client_id=eq.${client.id}&order=created_at.desc`)
      .then(data => { setFilings(data || []); setLoading(false); });
  }, [client?.id]);
  ...
  return (
    <div style={{ flex: 1, padding: 10, background: "#f2f5fa", overflowY: "auto" }}>
      {loading ? (
        <div style={{ textAlign: "center", padding: 24, fontSize: 11, color: "#8896b0" }}>Loading filing status…</div>
      ) : filings.length === 0 ? (
        <Card>
          <div style={{ textAlign: "center", padding: 12, fontSize: 11, color: "#8896b0" }}>
            No filing records found. Contact SRK team for status.
          </div>
        </Card>
      ) : (
        <>
          {pendingFilings.length > 0 && (
            <>
              <SL>Pending Returns</SL>
              <Card>
                {pendingFilings.map((f, i) => (
                  <div key={f.id} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "6px 0", borderBottom: i < pendingFilings.length - 1 ? "0.5px solid #f0f4fa" : "none" }}>
                    <div>
                      <div style={{ fontSize: 11, fontWeight: 600, color: "#1a2440" }}>{f.return_type} · {f.period}</div>
                      <div style={{ fontSize: 9, color: "#8896b0" }}>Due: {f.due_date || "—"}</div>
                    </div>
            ...
            <>
              ...
              <Card>
                {filedFilings.map((f, i) => (
                  <div key={f.id} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "6px 0", borderBottom: i < filedFilings.length - 1 ? "0.5px solid #f0f4fa" : "none" }}>
                    <div>
                      <div style={{ fontSize: 11, fontWeight: 600, color: "#1a2440" }}>{f.return_type} · {f.period}</div>
                      <div style={{ fontSize: 9, color: "#8896b0" }}>Filed: {f.filed_date || "—"} {f.arn ? `· ARN: ${f.arn}` : ""}</div>
                    </div>
                    <span className={bx(f.status)}>{f.status}</span>
                  </div>
                ))}
      ...
      <div style={{ background: "#fff8f0", border: "0.5px solid #fdd5b0", borderRadius: 8, padding: "7px 9px", fontSize: 9, color: "#92400e" }}>
        ⚠️ Status is updated live by SRK team. Contact SRK for any queries.
      </div>
    </div>
  );
}

// ── Payment screen (LIVE from Supabase payments table) ────────────────────────
function PaymentScreen({ client, payments }) {
  ...
  return (
    <div style={{ flex: 1, padding: 10, background: "#f2f5fa", overflowY: "auto" }}>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 6, marginBottom: 8 }}>
        {[["Total Billed", total ? `₹${Math.round(total / 1000)}K` : "—", N], ["Collected", paid ? `₹${Math.round(paid / 1000)}K` : "—", "#0a7a4f"], ["Pending", pending ? `₹${Math.round(pending / 1000)}K` : "Nil", O]].map(([l, v, c]) => (
      ...
      {total > 0 && (
        <Card>
          <SL>Collection Progress</SL>
          <div style={{ background: "#f0f3fa", borderRadius: 6, height: 7, overflow: "hidden", marginBottom: 4 }}>
            <div style={{ width: pct + "%", height: "100%", background: O, borderRadius: 6 }} />
          </div>
          <div style={{ display: "flex", justifyContent: "space-between", fontSize: 9 }}>
            <span style={{ color: "#8896b0" }}>{pct}% collected · FY 2025-26</span>
            <span style={{ color: O, fontWeight: 700 }}>₹{pending.toLocaleString("en-IN")} due</span>
          </div>
        </Card>
      )}
      <Card>
        ...
        {payments.length === 0 ? (
          <div style={{ fontSize: 11, color: "#8896b0", textAlign: "center", padding: 12 }}>No invoices raised yet.</div>
        ) : (
          payments.map((inv, i) => (
            <div key={inv.id} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "6px 0", borderBottom: i < payments.length - 1 ? "0.5px solid #f0f4fa" : "none" }}>
              ...
              <div style={{ textAlign: "right" }}>
                <div style={{ fontSize: 11, fontWeight: 700, color: inv.status === "Unpaid" ? O : "#1a2440" }}>₹{Number(inv.amount).toLocaleString("en-IN")}</div>
                <span className={bx(inv.status)}>{inv.status}</span>
              </div>
            </div>
          ))
        )}
      </Card>
      <button onClick={() => alert("Contact SRK Tax Audit\n📞 7418040882\n📧 srktaxaudit@yahoo.com")} style={{ width: "100%", padding: 11, background: O, color: "#fff", border: "none", borderRadius: 11, fontSize: 12, fontWeight: 700, cursor: "pointer" }}>
        💳 Pay Now — Contact SRK
      </button>
    </div>
  );
    ...
    <div style={{ flex: 1, padding: 10, background: "#f2f5fa", overflowY: "auto" }}>
      <div style={{ background: "#fff", borderRadius: 10, padding: "8px 10px", border: "0.5px solid #e6ecf5", borderLeft: "3px solid #b91c1c", marginBottom: 8 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <div>
            <div style={{ fontSize: 11, fontWeight: 600, color: "#1a2440" }}>TDS Challan 281</div>
            <div style={{ fontSize: 9, color: "#8896b0" }}>Due: 07 Apr 2026 — Urgent!</div>
          </div>
      ...
      <Card>
        {DUES.map((d, i) => (
          <div key={i} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "6px 0", borderBottom: i < DUES.length - 1 ? "0.5px solid #f0f4fa" : "none" }}>
            <div>
              <div style={{ fontSize: 11, fontWeight: 600, color: "#1a2440" }}>{d.t}</div>
              <div style={{ fontSize: 9, color: "#8896b0" }}>{d.d} · {d.f}</div>
            </div>
            <div style={{ fontSize: 8, fontWeight: 700, padding: "3px 7px", borderRadius: 6, background: d.days <= 5 ? "#fde8e6" : d.days <= 10 ? "#fef3c7" : "#e8f7f1", color: d.days <= 5 ? "#b91c1c" : d.days <= 10 ? "#92400e" : "#065f46" }}>
              {d.days}d
            </div>
          </div>
        ))}
      ...
      <div style={{ background: "#fff8f0", border: "0.5px solid #fdd5b0", borderRadius: 8, padding: "7px 9px", fontSize: 9, color: "#92400e" }}>
        ⚠️ Contact SRK immediately for TDS Challan and GSTR-7
      </div>
    </div>
  );
}
      ...
      {NOTICES_DATA.filter(n => n.status !== "Closed").map((n, i) => (
        <div key={i} style={{ background: "#fff", borderRadius: 10, padding: "9px 10px", marginBottom: 8, borderLeft: `3px solid ${n.color}`, border: "0.5px solid #e6ecf5", borderLeft: `3px solid ${n.color}` }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 5 }}>
            <div>
              <div style={{ fontSize: 11, fontWeight: 700, color: "#1a2440" }}>{n.title}</div>
              <div style={{ fontSize: 9, color: "#8896b0" }}>{n.sub}</div>
            </div>
            <span className={bx(n.status)}>{n.status}</span>
          </div>
          {[["Demand", n.demand, n.color], ["Reply Due", n.due, "#1a2440"], ["SRK Status", n.srk, n.status === "Action Req" ? O : "#065f46"]].map(([l, v, c]) => (
            <div key={l} style={{ display: "flex", justifyContent: "space-between", fontSize: 9, padding: "2px 0" }}>
              <span style={{ color: "#8896b0" }}>{l}</span>
...
// ── Inbox screen (LIVE messages from admin) ───────────────────────────────────
function InboxScreen({ client }) {
  ...
  useEffect(() => {
    if (!client?.id) { setLoading(false); return; }
    db(`messages?client_id=eq.${client.id}&order=created_at.desc`)
  ...
  if (loading) return <div style={{ flex: 1, padding: 10, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 11, color: "#8896b0" }}>Loading inbox…</div>;
  ...
  return (
    <div style={{ flex: 1, padding: 10, background: "#f2f5fa", overflowY: "auto" }}>
      ...
      <Card>
        {unread.length === 0 ? (
          <div style={{ fontSize: 11, color: "#8896b0", textAlign: "center", padding: 12 }}>No new messages.</div>
        ) : (
          unread.map((m, i) => (
      ...
      {read.length > 0 && (
        <>
          ...
          <Card>
            {read.map((m, i) => (
              <div key={m.id} style={{ display: "flex", alignItems: "flex-start", gap: 8, padding: "6px 0", borderBottom: i < read.length - 1 ? "0.5px solid #f0f4fa" : "none" }}>
                <div style={{ width: 6, height: 6, borderRadius: "50%", background: "#dde3ee", marginTop: 4, flexShrink: 0 }} />
                <div style={{ flex: 1 }}>
                  <div style={{ fontSize: 11, fontWeight: 600, color: "#6b7a99" }}>{m.message.substring(0, 60)}{m.message.length > 60 ? "…" : ""}</div>
                  <div style={{ fontSize: 8, color: "#b0b8cc", marginTop: 1 }}>{m.staff_name} · {new Date(m.created_at).toLocaleDateString("en-IN")}</div>
                </div>
              </div>
            ))}
          </Card>
      ...
      <div style={{ flex: 1, padding: "10px", overflowY: "auto", display: "flex", flexDirection: "column", gap: 6 }}>
        {msgs.map((m, i) => (
          <div key={i} style={{ display: "flex", justifyContent: m.from === "user" ? "flex-end" : "flex-start" }}>
            <div style={{ background: m.from === "user" ? N : "#fff", color: m.from === "user" ? "#fff" : "#1a2440", borderRadius: m.from === "user" ? "10px 10px 2px 10px" : "10px 10px 10px 2px", padding: "8px 10px", maxWidth: "82%", fontSize: 10, border: m.from === "bot" ? "0.5px solid #e6ecf5" : "none" }}>
              {m.text}
            </div>
          </div>
        ))}
...
function VisionScreen({ client, setUploads }) {
  ...
  function handleFile(fl) {
    if (!fl.length) return;
    setScanned(false); setUploaded(false);
    ...
    <div style={{ flex: 1, padding: 10, background: "#f2f5fa", overflowY: "auto" }}>
      ...
      <Card>
        <div style={{ background: "#f0f4fa", borderRadius: 9, height: 90, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", marginBottom: 8, cursor: "pointer" }} onClick={() => fileRef.current?.click()}>
          ...
          {!scanned ? (
            <><div style={{ fontSize: 22, marginBottom: 4 }}>📷</div><div style={{ fontSize: 10, fontWeight: 600, color: N }}>Tap to scan document</div><div style={{ fontSize: 9, color: "#8896b0", marginTop: 1 }}>Or upload from gallery</div></>
          ) : (
            <><div style={{ fontSize: 22, marginBottom: 4 }}>✅</div><div style={{ fontSize: 10, fontWeight: 600, color: "#0a7a4f" }}>Scan complete!</div></>
          )}
        </div>
      ...
      {scanned && !uploaded && (
        <>
          ...
          <div style={{ background: "#e8f7f1", borderRadius: 9, padding: "8px 10px", marginBottom: 8 }}>
            {Object.entries({ Vendor: EX.vendor, "Invoice No": EX.invoice, Amount: EX.amount, "GST Amount": EX.gst, Date: EX.date, Status: "Extracted ✓" }).map(([l, v]) => (
              <div key={l} style={{ display: "flex", justifyContent: "space-between", fontSize: 10, padding: "2px 0" }}>
                <span style={{ color: "#8896b0" }}>{l}</span><span style={{ fontWeight: 600, color: l === "Status" ? "#0a7a4f" : "#1a2440" }}>{v}</span>
              </div>
            ))}
          </div>
...
function MoreScreen({ setTab }) {
  const items = [
    ["⚠️", "GST Notices", "DRC-01, ASMT-10, SCN", "notices", "#b91c1c"],
    ["📬", "Inbox", "Messages from SRK team", "inbox", N],
    ["🤖", "Ask SRK", "AI-powered GST assistant", "ask", "#7c3aed"],
    ["📷", "Vision", "Scan bills, extract data", "vision", "#0c447c"],
    ["📞", "Contact SRK", "7418040882 · srktaxaudit@yahoo.com", "contact", "#0a7a4f"],
  ];
  return (
    <div style={{ flex: 1, padding: 10, background: "#f2f5fa", overflowY: "auto" }}>
      <SL>More Features</SL>
      {items.map((item, i) => (
        <button key={i} onClick={() => item[3] === "contact" ? alert("SRK Tax Audit\n📞 7418040882\n📧 srktaxaudit@yahoo.com\n🌐 www.srktaxaudit.com") : setTab(item[3])} style={{ width: "100%", display: "flex", alignItems: "center", gap: 10, background: "#fff", borderRadius: 11, padding: "10px 12px", border: "0.5px solid #e6ecf5", marginBottom: 7, cursor: "pointer", textAlign: "left" }}>
          ...
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: 12, fontWeight: 700, color: "#1a2440" }}>{item[1]}</div>
            <div style={{ fontSize: 9, color: "#8896b0", marginTop: 1 }}>{item[2]}</div>
          </div>
...
export default function App() {
  ...
  async function sendOtp() {
    ...
    if (!cl || !cl.length) {
      cl = await db("clients", { method: "POST", body: JSON.stringify({ name: "Client " + phone.slice(-4), phone, email: "" }) });
    }
  ...
  async function verify() {
    ...
    if (client) {
      const [t, p, u] = await Promise.all([
        db(`tasks?client_id=eq.${client.id}&order=due_date.asc`),
        db(`payments?client_id=eq.${client.id}&order=created_at.desc`),
        db(`uploads?client_id=eq.${client.id}&order=uploaded_at.desc`),
      ]);
  ...
  function logout() {
    setStep(0); setClient(null); setPhone(""); setOtp(""); setTasks([]); setPayments([]); setUploads([]); setTab("home");
  }
    ...
    <div style={{ minHeight: "100dvh", background: "#f2f5fa", display: "flex", flexDirection: "column" }}>
      <div style={{ background: N, padding: "48px 20px 24px", display: "flex", flexDirection: "column", alignItems: "center", gap: 10 }}>
        ...
        <div style={{ textAlign: "center" }}>
          <div style={{ color: "#fff", fontSize: 18, fontWeight: 700 }}>SRK TAX AUDIT</div>
          <div style={{ color: "rgba(255,255,255,0.5)", fontSize: 11, marginTop: 2 }}>Pillars of Your Business</div>
        </div>
      </div>
      <div style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", padding: "20px 24px" }}>
        <div style={{ width: "100%", maxWidth: 360, background: "#fff", borderRadius: 20, padding: 24, border: "0.5px solid #e6ecf5" }}>
          ...
          <div style={{ textAlign: "center", fontSize: 10, color: "#b0b8cc", marginTop: 14 }}>
            Any 10-digit number · OTP: <strong style={{ color: "#6b7a99", fontFamily: "monospace" }}>123456</strong>
          </div>
        </div>
      </div>
  ...
  const HDRS = {
    home: { title: null, showClient: true },
    upload: { title: "Upload Documents", sub: "Secure submission" },
    returns: { title: "GST Returns", sub: "Filing status" },
    payment: { title: "Professional Fees", sub: "FY 2025-26" },
    dues: { title: "Due Dates", sub: "Apr 2026" },
    notices: { title: "GST Notices", sub: "ASMT/DRC/SCN" },
    inbox: { title: "Inbox", sub: "Messages from SRK" },
    ask: { title: "Ask SRK", sub: "AI assistant" },
    vision: { title: "Vision", sub: "Scan bills" },
    more: { title: "More", sub: "All features" },
  };
